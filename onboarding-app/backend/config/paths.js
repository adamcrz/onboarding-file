...
const archivePathFor = (storedPath, meta = null) => {
  ...
  // Every document says which it is. A contract the app generated and the one
  // the client signed and sent back are the same document under the same name,
  // and telling them apart by opening each one is exactly what an archive is
  // supposed to save you from. APPROVED is Compliance's sign-off, which is a
  // different fact again from a signature being on the page.
  //
  //   BLANK    — generated by the app; nobody has signed or returned it
  //   SIGNED   — the version that came back, signed
  //   APPROVED — Compliance has signed it off
  const marks = [
    meta.signed ? 'SIGNED' : 'BLANK',
